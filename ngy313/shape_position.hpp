...
float shape_position_at_x(const Drawable &drawable, const std::size_t at) {
  return vertex_member_at<rhw_position_t>(pstade::oven::at(detail::drawable_core_access::copy_vertex(drawable), at)).x;
}

template <typename Drawable>
float shape_position_at_y(const Drawable &drawable, const std::size_t at) {
  return vertex_member_at<rhw_position_t>(pstade::oven::at(detail::drawable_core_access::copy_vertex(drawable), at)).y;
}

template <typename Drawable>
float shape_center_x(const Drawable &drawable) {
  return pstade::oven::accumulate(detail::drawable_core_access::copy_vertex(drawable),
                                  0.f,

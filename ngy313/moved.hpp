...
struct moved_adaptor : public vertex_adaptor<Drawable> {
  moved_adaptor(const Drawable &drawable, const float move_x, const float move_y)
      : vertex_adaptor(drawable, pstade::oven::transformed(move_position(move_x, move_y))) {}
};

template <typename Drawable>
struct moved_at_adaptor : public vertex_adaptor<Drawable> {
  moved_at_adaptor(const Drawable &drawable, const float move_x, const float move_y, const std::size_t at)
      : vertex_adaptor(drawable, at, move_position(move_x, move_y)) {}
};
